...
Fonctions::Fonctions(int n)
{
	...
	for (int i = 2; i< 2*n; i++) {
		logcum[i] = logcum[i - 1] + logl(static_cast<double>(i + 1));
	}
}
...
long double Fonctions::comb(int n, int p) //computes n choose p
{
	if (p == 0) {
		return 1.0;
	}
...
long double Fonctions::combno(int n, int p) //computes  (n+p-1) choose p
{
	return comb(n + p - 1, p);
}

long double Fonctions::Pno(int p, int n, int t, double v) // p/(v+p)P(X=v) if X follows hyper-geometric distribution with parameters K=v+p, N=n+t+p-1 and n=t
{
	...
	else
	{
		...
		if (a == 0 || b == 0 || c == 0)
			return 0.0;
		return expl(static_cast<double>(logl(a) + logl(b) - logl(c)));
...
long double Fonctions::Phyper(int p, int n, int t, int v) // P(X=v) if X follows hyper-geometric distribution with parameters K=p, N=n+p and n=t
{
